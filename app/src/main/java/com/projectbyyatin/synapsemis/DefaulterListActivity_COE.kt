...
class DefaulterListActivity_COE : AppCompatActivity() {
    ...
    private lateinit var adapter: DefaulterAdapter   // reuse existing adapter
        ...
        db.collection("classes")
            ...
            .addOnSuccessListener { snapshot ->
                ...
                snapshot.documents.forEach { doc ->
                    val cls = doc.toObject(Class::class.java)
                    if (cls != null) {
                        cls.id = doc.id   // ← manually assign document ID
                        classList.add(cls)
                    }
                }
                updateClassSpinner()
