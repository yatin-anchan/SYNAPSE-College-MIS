...
class GenerateResultActivity : AppCompatActivity() {
    ...
    private val departments    = mutableListOf<DeptItem>()
    ...
    private fun loadDepartments() {
        showLoading(true)
        firestore.collection("departments").whereEqualTo("isActive", true).get()
            .addOnSuccessListener { docs ->
                showLoading(false)
                departments.clear()
                departments.add(DeptItem("", "Select Department"))
                docs.forEach { d -> departments.add(DeptItem(d.id, d.getString("name") ?: d.id)) }
                departmentSpinner.adapter = ArrayAdapter(
                    this, android.R.layout.simple_spinner_dropdown_item, departments.map { it.name })
                departmentSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    override fun onNothingSelected(p: AdapterView<*>?) {}
                    override fun onItemSelected(p: AdapterView<*>?, v: View?, pos: Int, id: Long) {
                        selectedDeptId = departments[pos].id
                        if (selectedDeptId.isNotEmpty()) loadCourses(selectedDeptId)
                        else resetAll()
                    }
                }
            }
    ...
    private fun loadCourses(deptId: String) {
        firestore.collection("courses")
            .whereEqualTo("departmentId", deptId).whereEqualTo("isActive", true).get()
            .addOnSuccessListener { docs ->
                courses.clear()
    ...
    private fun loadPublishedExams() {
        ...
        if (selectedDeptId.isEmpty())   { toast("Select a department"); return }
            ...
            .addOnSuccessListener { docs ->
                ...
                docs.forEach { doc ->
                    ...
                    when (exam.examType.lowercase()) {
                        ...
                        else                  -> written.add(item)   // Written, Combined
                    }
                }
    ...
    private fun populateChips(
        ...
        if (items.isEmpty()) { emptyText.visibility = View.VISIBLE; return }
    ...
    private fun updateGenerateButton() {
        // Practical is optional — written OR internal must have at least one
    ...
    private fun validateAndGenerate() {
        ...
        startActivity(Intent(this, ResultDisplayActivity::class.java).apply {
            ...
            putExtra("DEPT_NAME",          departments.find { it.id == selectedDeptId }?.name ?: "")
    ...
    private fun resetAll() {
        courses.clear()
        courseSpinner.adapter = ArrayAdapter(this,
            android.R.layout.simple_spinner_dropdown_item, listOf("Select Course"))
        resetExamCards()
    }
    ...
    private fun resetExamCards() {
        ...
        selectedWrittenExamIds.clear(); selectedInternalExamIds.clear(); selectedPracticalExamIds.clear()
        writtenExamsCard.visibility   = View.GONE
        internalExamsCard.visibility  = View.GONE
        practicalExamsCard.visibility = View.GONE
        generateButton.isEnabled = false
    ...
    private fun showLoading(show: Boolean) { loadingProgress.visibility = if (show) View.VISIBLE else View.GONE }
    private fun toast(msg: String) = Toast.makeText(this, msg, Toast.LENGTH_SHORT).show()

    private fun tsLong(v: Any?): Long = when (v) {
        is Timestamp -> v.toDate().time; is Long -> v
        is Double -> v.toLong(); is Int -> v.toLong(); else -> 0L
    }
    ...
    private fun buildExamFromRaw(id: String, raw: Map<String, Any>): Exam {
        ...
        (raw["subjects"] as? List<*>)?.forEach { item ->
            ...
            parsedSubjects.add(ExamSubject(
                subjectId = s["subjectId"] as? String ?: "", subjectName = s["subjectName"] as? String ?: "",
                subjectCode = s["subjectCode"] as? String ?: "", courseId = s["courseId"] as? String ?: "",
                courseName = s["courseName"] as? String ?: "", examDate = tsLong(s["examDate"]),
                examType = s["examType"] as? String ?: "Written", maxMarks = tsLong(s["maxMarks"]).toInt(),
        ...
        return Exam(
            id = id, examName = raw["examName"] as? String ?: "",
            semester = tsLong(raw["semester"]).toInt(), academicYear = raw["academicYear"] as? String ?: "",
            startDate = tsLong(raw["startDate"]), examType = raw["examType"] as? String ?: "",
            courseId = raw["courseId"] as? String ?: "", courseName = raw["courseName"] as? String ?: "",
            isActive = raw["isActive"] as? Boolean ?: true,
            courses = (raw["courses"] as? List<*>)?.filterIsInstance<String>() ?: emptyList(),
            subjects = parsedSubjects, marksEntryEnabled = raw["marksEntryEnabled"] as? Boolean ?: false,
            marksPublished = raw["marksPublished"] as? Boolean ?: false, subjectsMarksStatus = parsedStatus
        )
    }

    data class DeptItem(val id: String, val name: String)
